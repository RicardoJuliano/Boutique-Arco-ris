...
// Postal zones from Buenopolis/MG - Correios 2025 table used by the storefront.
const ZONES = {
...
function getWeightIndex(itemCount) {
  const kg = Math.max(1, Number(itemCount)) * 0.4;
  if (kg <= 0.5) return 0;
  if (kg <= 1.0) return 1;
  if (kg <= 2.0) return 2;
  return 3;
...
async function getFreightQuote(cep, itemCount = 1) {
  const cleanCep = String(cep || '').replace(/\D/g, '');

  if (cleanCep.length !== 8) {
    const err = new Error('CEP invalido. Informe 8 digitos.');
    err.status = 400;
    throw err;
  }
  ...
  if (!response.ok) {
    const err = new Error('Servico de CEP indisponivel. Tente novamente.');
    err.status = 502;
    throw err;
  }
  ...
  if (data.erro) {
    const err = new Error('CEP nao encontrado. Verifique e tente novamente.');
    err.status = 404;
    throw err;
  }
  ...
  const weightIndex = getWeightIndex(itemCount);
  ...
  return {
    ...
    shipping: {
      pac: {
        ...
        label: `PAC - ${zoneConfig.pacDays[0]} a ${zoneConfig.pacDays[1]} dias uteis`,
        days: zoneConfig.pacDays[1],
      },
      sedex: {
        code: '04014',
        price: SEDEX_PRICES[zoneConfig.zone][weightIndex],
        label: `SEDEX - ${zoneConfig.sedexDays[0]} a ${zoneConfig.sedexDays[1]} dias uteis`,
...
module.exports = { getFreightQuote };
